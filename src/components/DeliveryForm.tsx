...
export default function DeliveryForm({ onSubmit }: DeliveryFormProps) {
	...
	useEffect(() => {
		if (phoneNumber || deliveryType || deliveryDate || deliveryLocation) {
	...
	}, [phoneNumber, deliveryType, deliveryDate, deliveryLocation])

	const selectedDeliveryOption = DELIVERY_TYPES.find(
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div className="relative">
						...
						<div className="relative">
							<button
								type="button"
								className="w-full px-4 py-3 text-black text-left bg-gray-50 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#6150FF] focus:border-transparent text-gray-900 focus:text-[#05054E]"
								onClick={() => setIsDropdownOpen(!isDropdownOpen)}
							>
								{deliveryType ? (
									<div className="flex justify-between items-center">
										<span className="text-[#151515]">{deliveryType}</span>
										<span className="text-[#151515]">
											{selectedDeliveryOption?.price
												? `GHS ${selectedDeliveryOption.price.toFixed(2)}`
												: ''}
										</span>
									</div>
								) : (
									...
									{DELIVERY_TYPES.map((option) => (
										<button
											...
											className={`w-full px-4 py-3 text-left hover:bg-gray-50 focus:outline-none focus:bg-gray-50 ${
												option.comingSoon ? 'cursor-not-allowed text-gray-400' : 'text-gray-900'
											}`}
											onClick={() => {
										...
										>
											<div className="flex justify-between items-center">
												...
												{option.price && <span className="text-gray-900">GHS {option.price.toFixed(2)}</span>}
											</div>
										</button>
									))}
								</div>
						...
						<div>
							...
							<div className="relative">
								<input
									...
									min={minDateString}
									className="w-full px-4 py-3 bg-gray-50 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#6150FF] focus:border-transparent text-gray-900 focus:text-[#05054E]"
									required
								/>
					...
					<div>
						...
						<div className="relative">
							<input
								...
								className="w-full px-4 py-3 bg-gray-50 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#6150FF] focus:border-transparent text-gray-900 focus:text-[#05054E] pr-10"
								required
							/>
